...
Route::group(['as' => 'lawyer.', 'prefix' => 'lawyer'], function () {
    ...
    Route::get('cause-list-print', function () {
        ...
        Route::get('cause/create', function () {
            ...
            if(request()->prev_day){
                $check = CaseProceeding::distinct()->orderBy('updated_next_date', 'asc')->whereDate('updated_next_date', '<=', Carbon::parse(request()->prev_day)->addDay(-1))->pluck('updated_next_date')->toArray();
                if($check && $check['0']){
                    $dates = CaseProceeding::distinct()->orderBy('updated_next_date', 'asc')->whereDate('updated_next_date', '>=', Carbon::parse($check['0']))->pluck('updated_next_date')->toArray();
                     return view('lawyer.cause-list.all',compact('dates'));
                }
                $dates = CaseProceeding::distinct()->orderBy('updated_next_date', 'asc')->whereDate('updated_next_date', '>=', Carbon::parse(request()->prev_day)->addDay(-1))->pluck('updated_next_date')->toArray();
                return view('lawyer.cause-list.all',compact('dates'));
            }
            if(request()->next_day){
                $check = CaseProceeding::distinct()->orderBy('updated_next_date', 'asc')->whereDate('updated_next_date', '>=', Carbon::parse(request()->next_day)->addDay(+1))->pluck('updated_next_date')->toArray();
                if($check && $check['0']){
                    $dates = CaseProceeding::distinct()->orderBy('updated_next_date', 'asc')->whereDate('updated_next_date', '>=', Carbon::parse($check['0']))->pluck('updated_next_date')->toArray();
                    return view('lawyer.cause-list.all',compact('dates'));
                }
                ...
                $dates = CaseProceeding::distinct()->orderBy('updated_next_date', 'asc')->whereDate('updated_next_date', '>=', Carbon::today())->pluck('updated_next_date')->toArray();
                return view('lawyer.cause-list.all',compact('dates'));
        });

        Route::get('litigation-calender-list',[CasesController::class, 'litigation_calender_list'])->name('litigation-calender-list');
        Route::get('litigation-calender-short',[CasesController::class, 'litigation_calender_short'])->name('litigation-calender-short');

        // Route::get('/calendar', [CasesController::class, 'getEvent'])->name('getevent');
        // Route::get('/getevent', [CasesController::class, 'getEvent']);
        // Route::post('/createevent', [CasesController::class, 'createEvent'])->name('createevent');
        // Route::post('/deleteevent', [CasesController::class, 'getEvent'])->name('deleteevent');
